/*
    -- MAGMA (version 2.3.0) --
       ...
       @generated from sparse/blas/zgecscsyncfreetrsm.cu, normal z -> c, Wed Nov 15 00:34:24 2017
       @author Weifeng Liu
...
void sptrsv_syncfree_analyser(magmaIndex_ptr         d_cscRowIdx,
                              magmaFloatComplex_ptr d_cscVal,
                              magma_int_t            m,
                              magma_int_t            nnz,
                              ...
                              magmaFloatComplex_ptr d_x)
{
    ...
    const magmaFloatComplex coef = one / d_cscVal[pos];
    ...
    if (opt == MAGMA_CSC_SYNCFREE_OPT_WARP_NNZ)
    {
                           ...
                           jj < stop_ptr; jj += MAGMA_CSC_SYNCFREE_WARP_SIZE)
        {
            ...
            for (magma_index_t k = 0; k < rhs; k++)
                atomicAddmagmaFloatComplex(&d_x[rowIdx * rhs + k], 
    ...
    magma_queue_t           queue )
{
    ...
    cudaMemset(dx, 0, sizeof(magmaFloatComplex) * m * rhs);
