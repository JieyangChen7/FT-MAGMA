	...
	*/
	...
	if(FT){
		...
		for (int i = 0; i < m; i += n) {
			...
			double * chk1 = chk +(i/n)*2;
			double * chk2 = chk1 + 1;
			cublasDgemv(handle, CUBLAS_OP_T, n, n, &one, C + i, ldc, vd, 1, &zero, chk1, chk_ld);
			cublasDgemv(handle, CUBLAS_OP_T, n, n, &one, C + i, ldc, v2, 1, &zero, chk2, chk_ld);
		}
