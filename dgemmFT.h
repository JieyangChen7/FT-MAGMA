	...
	*/
	...
	if(FT){
		...
		for (int i = 0; i < m; i += n) {
			...
			cublasDgemv(handle, CUBLAS_OP_T, n, n, &one, C + i, ldc, vd, 1, &zero, chk +(i/n)*2, chk_ld);
			cublasDgemv(handle, CUBLAS_OP_T, n, n, &one, C + i, ldc, vd + vd_ld, 1, &zero, chk +(i/n)*2+1, chk_ld);
		}
