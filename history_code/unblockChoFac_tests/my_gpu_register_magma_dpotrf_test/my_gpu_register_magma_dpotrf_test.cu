...
__global__ void POTF2_register_magma(double * matrix, int ld){
    register  double localMatrix[b_size*b_size];
    //int b_size = 256;
...
int main(){
    ...
    // for(int n=2;n<1025;n*=2){
        for(int i=0;i<TEST_NUM;i++){
            test(0,b_size,2,'u',&real_time,&proc_time,&flpins,&mflops);
            total_real_time += real_time;
            total_proc_time += proc_time;
            total_flpins += flpins;
            total_mflops += mflops;
        }
        cout<<"GPU REGISTER MAGMA:";
        cout<<"Size:"<<b_size<<"---Real_time:"<<total_real_time/(double)TEST_NUM<<"---"<<"Proc_time:"<<total_proc_time/(double)TEST_NUM<<"---"<<"Total flpins:"<<total_flpins/(double)TEST_NUM<<"---"<<"MFLOPS:"<<total_mflops/(double)TEST_NUM<<endl;
	// }
