 /*Blocked Cholesky Factorization v1.4.1y
 *This version tyring compress the memory usage, incomplete version, cublas dose not support. 
...
void POTF2(double * matrix, int I, int N, int B){
    for(int i = I; i<I+B;i++){
      setElement(matrix,N,i,i,sqrt(getElement(Matrix,N,i,i)));
      for(int j=i+1;j<I+B;j++)
	setElement(matrix,N,j,i,getElement(matrix,N,j,i)/getElement(matrix,N,i,i));
      for(int j=i+1;j<I+B;j++)
	for(int k=i+1;k<j+1 )
...
double getElement(double * matrix, int N, int i, int j){
  return matrix[(1+i)*i/2+j];
}

void setElement(double * matrix, int N, int i, int j, double value){
  matrix[(1+i)*i/2+j] = value;
}
...
void matrixGenerator(double * matrix, double * result, int N){
  ...
  for(int i=0;i<N;i++){
    for(int j=i;j<N;j++){
      double sum =0.0;
      for(int k=0;k<i+1;k++){
	sum+=getElement(result,N,j,k)*getElement(result,N,i,k);
      }
      setElement(matrix,N,j,i,sum);
    }
  }  
