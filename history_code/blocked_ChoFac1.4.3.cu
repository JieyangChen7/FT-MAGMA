 /*Blocked Cholesky Factorization v1.4.4
 *potf only cpu, dtrsm only gpu, dgemm only cublas
 *using leading dimension, compute both upper and lower, double the computation time. Initial data is on RAM, so transfer to GPU is taken acre of. 
