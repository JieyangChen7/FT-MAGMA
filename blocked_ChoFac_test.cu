...
void matrixGenerator_gpu(char uplo, double * matrix, int matrix_ld, double * result, int result_ld, int N,  int B){
    ...
    if(cublasStatus != CUBLAS_STATUS_SUCCESS)
      cout<<"CUBLAS NOT INITIALIZED(handle1) in matrixGenerator_gpu"<<endl;
...
void my_dpotrf(char uplo, double * matrix, int ld, int N, int B,float * real_time, float * proc_time, long long * flpins, float * mflops){
  ...
  if(cublasStatus != CUBLAS_STATUS_SUCCESS)
    cout<<"CUBLAS SET STREAM NOT INITIALIZED(handle1) in my_dpotrf"<<endl;
  ...
  for(int i=0;i<N;i+=B){
	...
	//double * matrix_i_ld_i = matrix_i_ld+i;
	//double * matrix_i_b_size_ld = matrix+(i+b_size)*ld;
	//double * matrix_i_b_size_ld_i = matrix_i_b_size_ld+i;
	...
	*/
	...
	cudaMemcpy2DAsync(matrix+i*ld+i,
			  ld*sizeof(double),
			  temp,
			  B*sizeof(double),
			  B*sizeof(double),
