...
void my_dpotrf(char uplo, double * matrix, int ld, int N, int B,
	...
	for (int i = 0; i < N; i += B) {
		...
		cudaMemcpy2DAsync(chk1, B * sizeof(double), checksum1 + (i/B) + (i/B)*checksum1_ld,
				checksum1_ld, 1 * sizeof(double), B,
				cudaMemcpyDeviceToHost, stream0);
		cudaMemcpy2DAsync(chk2, B * sizeof(double), checksum2 + (i/B) + (i/B)*checksum2_ld,
		...
		cudaMemcpy2DAsync(checksum1 + (i/B) + (i/B)*checksum1_ld,checksum1_ld, chk1, B * sizeof(double), 
				1 * sizeof(double), B,
				cudaMemcpyHostToDevice, stream0);
		cudaMemcpy2DAsync(checksum2 + (i/B) + (i/B)*checksum2_ld,checksum2_ld, chk2, B * sizeof(double), 
				1 * sizeof(double), B,
				cudaMemcpyHostToDevice, stream0);
