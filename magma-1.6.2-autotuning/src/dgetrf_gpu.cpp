    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        if (FT) {
            ...
            //col_benchmark_single(abftEnv, dAT, lddat);
              row_benchmark_single(abftEnv, dAT, lddat);
        ...
        for( j=0; j < s; j++ ) {
           // cout << "j=" << j << endl;
            ...
            dgetrfFT(rows, nb, work, ldwork, ipiv+j*nb, &iinfo, 
                     abftEnv, FT, DEBUG, VERIFY_BEFORE, VERIFY_AFTER
                     );
            ...
            if (false) {
                ...
                row_chk_swap(abftEnv, dAT, lddat, real_effect, stream);
